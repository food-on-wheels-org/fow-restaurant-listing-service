    ...
    stages {
        ...
        stage('Code Coverage') {
            steps{
                script{
                    ...
                    def response = sh(
                        script: """curl -H "Authorization: Bearer ${token}" "${sonarQubeUrl}/measures/api/component?component=${componentKey}&metricKeys=coverage" """,
                        returnStdout: true
                    ).trim()
        ...
        stage('Update Image tag in GitOps') {
            steps {
                ...
                script {
                       sh '''
                          sed -i "s/image:.*/image: tejassrivathsa\\/restaurant-listing-service:${VERSION}/" k8s/apps/restaurant-listing-manifest.yml
                        '''
